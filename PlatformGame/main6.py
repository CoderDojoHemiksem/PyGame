...
####################### STEP 5 #######################
# ADD COLLISION DETECTION + END GAME SCREEN WITH SCORE
######################################################
...
# New function to initialize the game
def init_game():
...
# New function for the end game screen!
def end_screen():
    end_run = True
    while end_run:
...
# New function to store the highest score
def update_score():
    last = 0
...
while running:
    ...
    # Check if any obstacles have collided with the runner          # New
    if pygame.sprite.spritecollideany(runner, obstacles):           # New
        # If so, then remove the player and stop the loop           # New
        runner.falling = True                                       # New
        pause = 1                                                   # New
